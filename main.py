...
async def webhook_mp(empresa_id: str, request: Request):
    ...
    try:
        ...
        if type_ == "merchant_order":
            # Guardar fecha actual en Argentina
            date_created = datetime.now(AR_TZ).strftime("%Y-%m-%d %H:%M:%S")
        ...
        nuevo_evento = Evento(
            ...
            action=action,
            type=type_,
            date_created=payment_info.get("date_created") if type_ == "payment" else date_created,
            payment_id=payment_info.get("payment_id") if type_ == "payment" else payment_id,
            status=payment_info.get("status") if type_ == "payment" else None,
            merchant_order_id=payment_info.get("merchant_order_id") if type_ == "payment" else (str(orden_id) if type_ == "merchant_order" else None),
            external_reference=payment_info.get("external_reference") if type_ == "payment" else None,
            contenido=payment_info.get("evento_json") if type_ == "payment" else payload,
            empresa_id=empresa.id
        )
        db.add(nuevo_eventfrom fastapi import FastAPI, Request, HTTPException
from fastapi.responses import JSONResponse
from sqlalchemy.exc import SQLAlchemyError, OperationalError
from sqlalchemy.orm import Session
from sqlalchemy import text, and_, or_
